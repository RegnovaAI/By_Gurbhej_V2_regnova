...
import { Search, User, ChevronLeft, FileText, AlertTriangle, CheckCircle, Edit, Shield, Loader2 } from 'lucide-react';
...
export default function AuditView() {
  ...
  const getRiskBg = (risk) => {
    switch (risk) {
      case 'high': return 'bg-red-800/80 border-red-500/40';
      case 'medium': return 'bg-yellow-800/80 border-yellow-500/40';
      case 'low': return 'bg-blue-800/80 border-blue-500/40';
    ...
    apiResults.forEach(result => {
      if (!seen.has(result.audit_type)) {
        types.push({
          ...
          status: (result.risk_report && result.risk_report.some(r => r.risk_level === 'High')) ? 'high'
            : (result.risk_report && result.risk_report.some(r => r.risk_level === 'Medium')) ? 'medium'
            : (result.risk_report && result.risk_report.some(r => r.risk_level === 'Low')) ? 'low'
            : 'missing',
          count: apiResults.filter(r => r.audit_type === result.audit_type).length
        });
  ...
  async function fetchProjects() {
    ...
    try {
      ...
      const res = await fetch(`${BASE_URL}/project/${router?.query?.projectId}/audit-report`, {
  ...
  const subheadingSize = "text-base sm:text-lg md:text-xl lg:text-xl xxl:text-2xl";
    ...
    <div className="flex flex-col w-screen h-screen lg:flex-row bg-gray-900 text-white">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-80 bg-gray-800 border-r border-gray-700 overflow-y-auto">
          <div className="p-6">
            <div className="flex items-center space-x-2 mb-6 cursor-pointer" onClick={() => router.push('/profile')}>
              <ChevronLeft className="w-5 h-5 text-gray-400 cursor-pointer hover:text-white" />
              <h2 className={`font-semibold ${headingSize}`}>RegnovaPilot<sup>TM</sup></h2>
            </div>
            {/* Risk Level Legend */}
            <div className="mb-6">
            ...
            <div className="space-y-2">
              {auditTypes.map((audit, index) => (
                <div key={index}>
                  <div
                    onClick={() => {
                      setExpandedAudit(expandedAudit === audit.name ? null : audit.name);
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex-1">
                        <div className={`font-medium text-sm ${fontSize}`}>{audit.name}</div>
                        <div className="flex items-center space-x-2 mt-1">
                          <div className={`w-2 h-2 ${getStatusColor(audit.status)} rounded-full`}></div>
                          <span className="text-base text-gray-400">{getStatusText(audit.status)}</span>
                        </div>
                      </div>
                      {audit.count > 0 && (
                        <div className={`px-2 py-1 text-base rounded-full ${getStatusColor(audit.status)} text-white`}>
                          {audit.count}
                        </div>
                      )}
                    </div>
                  ...
                  {expandedAudit === audit.name && auditDocuments[audit.name] && (
                    <div className="ml-4 mt-2 space-y-1">
                      {auditDocuments[audit.name].map(doc => (
                        <div
                          key={doc.id}
                          className={`cursor-pointer text-base py-1 ${selectedDocument && selectedDocument.id === doc.id ? 'text-blue-400 font-semibold' : ''}`}
                          onClick={() => {
            ...
            {apiLoading && (
              <div className="mt-4 text-blue-400 flex items-center gap-2">
                <Loader2 className="animate-spin w-4 h-4" /> Loading audit report...
              </div>
            )}
            {apiError && (
              <div className="mt-4 text-red-400">{apiError}</div>
            )}
          </div>
        </div>
          ...
          <div className="p-6">
            ...
            {Object.keys(frameworkScores).length > 0 && (
              <div className="mb-6 flex flex-wrap gap-4">
                {Object.entries(frameworkScores).map(([fw, score]) => (
                  <div key={fw} className="bg-gray-800 px-4 py-2 rounded-lg text-sm">
                    <span className="font-semibold">{fw}:</span> <span className="text-blue-400">{score}</span>
                  </div>
                ))}
              </div>
            )}
            ...
            <div className="mb-8">
              <div className="flex items-start justify-between">
                <div>
                  <h1 className={`font-semibold mb-2 ${headingSize}`}>{selectedAudit}</h1>
              ...
              <div className="space-y-6">
                ...
                {apiLoading ? (
                  ...
                  <>
                    ...
                    {selectedDocument && selectedDocument.clauses && selectedDocument.clauses.filter(c => c.risk !== 'missing').length > 0 ? (
                      selectedDocument.clauses
                        .filter(clause =>
                          riskTab === 'all'
                            ? clause.risk === 'high' || clause.risk === 'medium' || clause.risk === 'low'
                            : clause.risk === riskTab
                        )
                        .map((clause, idx) => (
                          <div
                            key={clause.id}
                            className={`rounded-lg p-6 border ${getRiskBg(clause.risk)} transition-all`}
                          >
                            <div className="flex items-start space-x-4">
                              <div className={`p-2 rounded-lg ${getRiskIconBg(clause.risk)}`}>
                                {clause.risk === 'high' ? (
                                  <AlertTriangle className={`w-6 h-6 ${getRiskIconColor(clause.risk)}`} />
                                ) : clause.risk === 'medium' ? (
                                  <Shield className={`w-6 h-6 ${getRiskIconColor(clause.risk)}`} />
                                ) : (
                                  <CheckCircle className={`w-6 h-6 ${getRiskIconColor(clause.risk)}`} />
                                )}
                              </div>
                              <div className="flex-1">
                                <h3 className={`font-semibold mb-2 ${subheadingSize} ${getRiskIconColor(clause.risk)}`}>
                                  {clause.risk.charAt(0).toUpperCase() + clause.risk.slice(1)} Risk
                                </h3>
                                <div className={`mb-2 text-gray-300 ${fontSize}`}>{clause.text}</div>
                                {clause.riskDetail && (
                                  <div className="text-base text-gray-400 mb-2">{clause.riskDetail}</div>
                                )}
                                {clause.suggestion && (
                                  <div className="mt-2">
                                    <span className="font-semibold text-blue-400">Recommendation: </span>
                                    <span className="text-gray-200">{clause.suggestion}</span>
                                    {clause.citation && (
                                      <span className="ml-2 text-base text-gray-400">({clause.citation})</span>
                                    )}
                                  </div>
                                )}
                                <div className="mt-2 text-base text-gray-400">
                                  <span>Status: {clause.validation_status || 'N/A'}</span>
                                </div>
                              </div>
                            </div>
                          </div>
                      ...
                      <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
                        <div className="flex items-start space-x-4">
                          ...
                          <div className="flex-1">
                            <h3 className={`font-semibold mb-2 text-green-400 ${subheadingSize}`}>No Risks Detected</h3>
                            <p className={`text-gray-300 ${fontSize}`}>No significant risks were found in this document.</p>
                          </div>
                        </div>
                      </div>
                    )}
                ...
                {apiLoading ? (
                  ...
                  selectedDocument && (
                    <div className="bg-gray-800 rounded-lg p-6 mt-6 border border-gray-700">
                      <h3 className={`font-semibold mb-4 text-blue-400 ${subheadingSize}`}>Clauses</h3>
                      <div className="space-y-2">
                        {selectedDocument.clauses.length === 0 && (
                          <div className="text-base text-gray-400">No clauses found.</div>
                        )}
                        {selectedDocument.clauses.map(clause => (
                          ...
                          >
                            <div className="flex items-center justify-between">
                              ...
                              <span className={`ml-2 px-2 py-1 rounded-full text-base ${
                                clause.risk === 'high'
                                  ? 'bg-red-500 text-white'
                                  : clause.risk === 'medium'
                                  ? 'bg-yellow-500 text-white'
                                ...
                                {clause.risk ? clause.risk.charAt(0).toUpperCase() + clause.risk.slice(1) : 'Unknown'}
                              </span>
                            </div>
                            {selectedClause && selectedClause.id === clause.id && (
                              <div className="mt-2 ml-2 text-base text-gray-300">
                                <div><strong>Suggestion:</strong> {clause.suggestion}</div>
                                <div><strong>Risk Detail:</strong> {clause.riskDetail}</div>
                                <div><strong>Citation:</strong> {clause.citation}</div>
                                <div><strong>Status:</strong> {clause.validation_status}</div>
                              </div>
                            )}
                          </div>
                        ))}
